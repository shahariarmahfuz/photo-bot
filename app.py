...
async def get_anime_number(update: Update, context: CallbackContext) -> int:
    ...
    await update.message.reply_text("🖼 *2:3 থাম্বনেইলের জন্য একটি ছবি পাঠান:*", parse_mode="MarkdownV2")
    return IMG_RATIO_2_3

async def get_img_ratio_2_3(update: Update, context: CallbackContext) -> int:
    img_url = await handle_photo(update, context)
    if img_url:
        context.user_data["img"] = img_url
        await update.message.reply_text("🎬 *16:9 থাম্বনেইলের জন্য একটি ছবি পাঠান:*", parse_mode="MarkdownV2")
...
async def get_img_ratio_16_9(update: Update, context: CallbackContext) -> int:
    img_url = await handle_photo(update, context)
    if img_url:
        context.user_data["anime_img"] = img_url
